...
export const insertFacturaForServicioTecnico = async (client, facturaData) => {
  const { id_cliente, costo, garantia_aplica, fecha_garantia, id_sede } = facturaData;
...
export const insertMetodoPagoForFactura = async (client, idFactura, metodo) => {
  if (metodo?.id_tipo && metodo?.monto > 0) {
    await client.query(
      `INSERT INTO METODOPAGO
          (id_factura_metodopago, id_tipometodopago_metodopago, monto_metodopago, estado_metodopago)
        VALUES ($1, $2, $3, $4)`,
      [idFactura, metodo.id_tipo, metodo.monto, metodo.estado ?? "A"]
    );
  }
};

export const insertServicioTecnico = async (client, servicioTecnicoData) => {
  const {
    id_sede,
    id_proveedor,
    id_cliente,
  ...
  const proveedorFinal = id_proveedor && id_proveedor.trim() !== "" ? id_proveedor : "PROV_TEMP_123";
...
export const fetchAllServiciosTecnicos = async () => {
...
export const fetchServicioTecnicoById = async (id) => {
  const result = await pool.query(
    `
    SELECT
      st.id_serviciotecnico,
...
export const updateServicioTecnicoData = async (client, id, data) => {
  const {
