...
export async function createInventarioLocal(req, res) {
  try {
    ...
    res.status(err.status || 500).json({ message: err.message || 'Error al registrar el inventario local' });
  }
}

export async function updateInventarioLocal(req, res) {
  try {
    const actualizado = await repo.update(req.params.inventarioLocalId, req.body);
...
export async function existeEnInventarioLocal(req, res) {
  try {
    ...
    res.status(500).json({ error: 'Error verificando existencia en inventario local' });
  }
}
