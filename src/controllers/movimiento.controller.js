...
export const getMovimientos = async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT ID_MOV, ID_TIPOMOV, ID_PRODUCTO, CANTIDAD_MOV, FECHA_MOVIMIENTO, ESTADO_MOVIMIENTO FROM MOVPRODUCTO"
    );
...
export const createMovimiento = async (req, res) => {
  try {
    const { id_tipomov, id_producto, cantidad_mov, fecha_movimiento, estado_movimiento } = req.body;
    ...
    const result = await pool.query(
      ...
      [id_tipomov, id_producto, cantidad_mov, fecha_movimiento, estado_movimiento]
    );
